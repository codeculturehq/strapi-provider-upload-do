...
class FileLocationConverter {
  ...
  getKey(file, includeHash) {
    const filename = includeHash ? `${file.hash}${file.ext}` : file.name;
  ...
  init: config => {
    ...
    return {
      upload: file => new Promise((resolve, reject) => {
        ...
        S3.upload({
            Key: converter.getKey(file, includeHash),
      ...
      delete: file => new Promise((resolve, reject) => {
        ...
        //--- Delete the file from the space
          S3.deleteObject({
              ...
              Key: converter.getKey(file, file.url.includes(hashKey)),
            },
